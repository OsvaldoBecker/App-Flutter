...
module.exports = {
    ...
    update: async (req, res) => {
        let obj = new Client(req.body);
        await Client.updateOne({ _id: obj._id }, obj, function (err) {
            (err ? res.status(400).send(err) : res.status(200).json(obj));
        });
    },

    delete: async (req, res) => {
        await Client.deleteOne({ _id: req.params.id }, function (err) {
